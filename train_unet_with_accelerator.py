...
def main(config):
    ...
    project_config = ProjectConfiguration(
        project_dir=config.data.train_output_dir,
        logging_dir=os.path.join(config.data.train_output_dir, "logs")
    )
    
    accelerator = Accelerator(
        gradient_accumulation_steps=4,
        mixed_precision="fp16" if config.run.mixed_precision_training else "no",
    ...
    # syncnet_eval_model.loadParameters("/home/tmpuser/onkar/LatentSync/train_models/syncnet/train-2025_04_06-00:05:28/checkpoints/checkpoint-5000.pt")
    ...
    if config.model.cross_attention_dim == 1280:
        whisper_model_path = "/home/tmpuser/onkar/LatentSync/checkpoints/large-v1.pt"
    ...
    denoising_unet, optimizer, train_dataloader, lr_scheduler = accelerator.prepare(
        denoising_unet, optimizer, train_dataloader, lr_scheduler
    )
    ...
    for epoch in range(first_epoch, num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(denoising_unet):
                ...
                accelerator.clip_grad_norm_(trainable_params, config.optimizer.max_grad_norm)
                optimizer.step()
                lr_scheduler.step()
                optimizer.zero_grad()
                ...
                if accelerator.is_main_process and (global_step % config.ckpt.save_ckpt_steps == 0):
                    # Unwrap model before saving
                    unwrapped_model = accelerator.unwrap_model(denoising_unet)
                    model_save_path = os.path.join(output_dir, f"checkpoints/checkpoint-{global_step}.pt")
                    state_dict = {
                        "global_step": global_step,
                        "state_dict": unwrapped_model.state_dict(),
                    }
                    accelerator.save(state_dict, model_save_path)
                ...
                if loss.item() == 'nan':
                    print(loss.item(), batch["video_path"])
